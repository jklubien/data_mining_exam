...
gutenberg_books = ['austen-emma.txt', 'austen-persuasion.txt',
...
def define_gender(name_input):
    ...
    them in order to create a training set
    """
...
first character is a letter. If it is we check if the
second character is a letter and so on.
If four characters in a row are letters we return them
as the publication year. This will clearly only work for
books written after the year 999.
"""
